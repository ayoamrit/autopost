...
def complete_upload_form(
    ...
    **kwargs,
    ...
    #-----PRIVATE DISMISSAL CODE------
    # Close the popup modal showing up on the screen after video upload
    # blocking the script from interacting with the descripton and post buttons
    try:
        logger.debug(green("Attempting to close popup modal if it appears..."))
        
        # We use get_by_text with 'Got it' which is the standard TikTok dismissal string.
        # Timeout is set to 5s to account for slow UI, rendering after upload.
        page.get_by_text("Got it").click(timeout=5000)
        logger.debug(green("Dismissed the popup modal successfully"))
        
        # A small buffer allows the 'fading' animation of the modal to finish
        # so the elements behind it become 'interactable' again.
        page.wait_for_timeout(1000)
    except Exception:
        
        # If the modal doesn't appear, we log it as a debug and move on.
        logger.debug(red("Modal did not appear or could not be dismissted."))
    #---------------------------------
    #-----PRIVATE DISMISSAL CODE------
